"""CuTeDSL GDN MTP Decode + FLUSH Kernel — WY-parallel state_and_output variant.

Copy of gdn_decode_bf16_wy_output_only with a Phase-5 FLUSH epilogue: fold the
first P (per-request, runtime int32[B] ``flush_steps``) rows of the 16-row
left-packed window [P committed | T draft | zero tail] into the state pool IN
PLACE, per the ReplaySSM raw-v-cache design (see the phase5_flush excalidraw):
  Step A  KHraw = k_norm @ H0^T      — piggybacks the Phase-3 H GEMM halves
  Step B  C = V - diag(exp(G_i))KHraw
  Step C  U = Tmat @ C               — T_P top-left = causal P-row solve, free
  Step D  Khat = diag(i<P ? exp(G_P - G_i) : 0) k_norm
  Step E  H0_new = exp(G_P) H0 + U^T Khat   — one rank-P GEMM + H0 re-read/store
Outputs (Phases 1-4) are BIT-IDENTICAL to the output-only kernel and never read
H0_new. Launch only on flush iterations (~1 in 4.5); the hot verify path keeps
using gdn_decode_bf16_wy_output_only. t_input = n_valid = 16 only.

Everything below this banner is the inherited output-only kernel history:
...
def gated_delta_rule_mtp(
    ...
    """GDN MTP decode + Phase-5 flush (state_and_output variant).

    Same WY math and outputs as ``gdn_decode_bf16_wy_output_only`` (bit-equal
    through Phase 4), plus a fused flush: for each request i, the first
    ``flush_steps[i]`` window rows (the committed / ring rows) are folded into
    ``initial_state_source`` IN PLACE (one bf16 rounding at the store).
    ``flush_steps[i] == 0`` leaves that request's state untouched. Launch this
    ONLY on flush iterations — the hot verify path should keep using
    ``gdn_decode_bf16_wy_output_only``.

    Restrictions vs the output-only wrapper: requires the full left-packed
    16-row window ([P_i committed | T draft | zero tail]; NO middle padding —
    ...
    # --- flush-variant contract ---
    assert flush_steps is not None, (
        "gdn_decode_bf16_wy_state_and_output: flush_steps (int32 [B]) is "
        "required — use gdn_decode_bf16_wy_output_only for plain verify."
    )
    assert flush_steps.dtype == torch.int32, (
        f"flush_steps must be int32; got {flush_steps.dtype}."
    )
    assert not disable_state_update, (
        "gdn_decode_bf16_wy_state_and_output: this kernel writes state for "
        "rows < flush_steps; pass disable_state_update=False."
    )
    assert initial_state_source.is_contiguous(), (
        "initial_state_source must be contiguous — the flush writes it in place."
    )
    assert q.shape[1] == 16, (
        f"state_and_output requires the full 16-row window; got T={q.shape[1]}."
    )
    ...
    h0 = initial_state_source  # asserted contiguous above; written IN PLACE
    flush_steps = flush_steps.contiguous()
...
__all__ = ["gated_delta_rule_mtp", "GdnFlushKernel", "K_DIM", "V_DIM_C"]
